...
test("Agent features: 动态发现的 skill 保持只读并注入清单", async () => {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "agentx-skill-agent-"));
...
test("Agent features: steering 同样经过 UserPromptSubmit hook，内部 context 不冒充用户原话", async () => {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "agentx-steer-"));
  let release!: () => void;
  let started!: () => void;
...
test("Agent features: steering 全被 hook 拦截时不会空跑额外模型轮", async () => {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "agentx-steer-block-"));
  let release!: () => void;
  let started!: () => void;
  const gate = new Promise<void>((resolve) => (release = resolve));
  const entered = new Promise<void>((resolve) => (started = resolve));
...
test("Agent features: 并行工具准备阶段抛错仍为每个 tool_call 生成配对结果", async () => {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "agentx-tool-pair-"));
  const bad: Tool = {
    readOnly: true,
...
test("Agent features: 部分流失败重试前发 turn_reset，历史只保留成功尝试", async () => {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "agentx-retry-"));
  let attempt = 0;
  const provider: Provider = {
    name: "flaky",
    async *stream(): AsyncIterable<StreamEvent> {
...
test("Agent features: 部分流最终失败也发 turn_reset，不把残影留在 UI", async () => {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "agentx-terminal-reset-"));
  const provider: Provider = {
...
test("Agent features: confirm 改写后的最终动作仍受 deny 规则约束", async () => {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "agentx-updated-policy-"));
  let runs = 0;
...
test("Agent features: PostToolUse 可观察错误，PreToolUse context 不丢失", async () => {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "agentx-hook-tool-"));
  const failing: Tool = {
    readOnly: true,
...
test("Agent features: 子 agent 串行 task 的用量汇总进父会话", async () => {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "agentx-subagent-"));
  const provider = scriptedProvider([
    assistant({
      type: "tool_call",
      id: "task-1",
...
test("Agent features: 子 agent 继承父级 PreToolUse，task 不能绕过写入拦截", async () => {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "agentx-subagent-hooks-"));
...
test("Agent features: 复用 ToolRegistry 时 task 注册与有状态工具实例互不污染", async () => {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "agentx-registry-clone-"));
...
test("SessionManager: done 事件回调中的新消息进入下一 drive，不丢失不乱序", async () => {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "agentx-done-race-"));
...
test("SessionManager: steering send Promise 等到输入已处理且持久化后才完成", async () => {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "agentx-steer-promise-"));
  const store = new SessionStore(path.join(dir, "sessions"));
  let release!: () => void;
...
test("SessionManager: interrupt 后同 tick 的新 send 进入下一 drive", async () => {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "agentx-interrupt-race-"));
  const store = new SessionStore(path.join(dir, "sessions"));
  let markStarted!: () => void;
  const started = new Promise<void>((resolve) => (markStarted = resolve));
  const calls: ChatMessage[][] = [];
...
test("SessionManager: 主输入 hook 阻塞期间的新消息不会随主输入丢失", async () => {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "agentx-hook-race-"));
  const store = new SessionStore(path.join(dir, "sessions"));
