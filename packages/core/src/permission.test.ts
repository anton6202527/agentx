...
test("权限: 复合命令逐段匹配 —— allow 要求每段命中", async () => {
  ...
  const ok = await engine.check(
    req({ ruleKey: "git status && git log", ruleParts: splitShellCommand("git status && git log") }),
  );
  assert.equal(ok.behavior, "allow");
  // 夹带 rm → 不能被 git 前缀放行
  const bad = await engine.check(
    req({ ruleKey: "git status && rm -rf /", ruleParts: splitShellCommand("git status && rm -rf /") }),
  );
...
test("权限: hookAllowed 跳过 confirm，但压不过 deny/ask", async () => {
  ...
  const allowed = await noConfirm.check(req({ toolName: "write", ruleKey: "a.ts", hookAllowed: true }));
  assert.equal(allowed.behavior, "allow");

  const withDeny = new PermissionEngine({ denyRules: ["Write"] });
  const denied = await withDeny.check(req({ toolName: "write", ruleKey: "a.ts", hookAllowed: true }));
...
test("splitShellCommand: 顶层操作符拆分，引号内不拆", () => {
  ...
  assert.deepEqual(splitShellCommand("cat f | grep x; pwd || ls"), ["cat f", "grep x", "pwd", "ls"]);
  assert.deepEqual(splitShellCommand("ls"), ["ls"]);
  assert.deepEqual(splitShellCommand("git status & rm x\nprintf ok"), ["git status", "rm x", "printf ok"]);
...
test("权限: shell 后台操作符与复杂语法不能绕过细粒度 allow", async () => {
  ...
  const d1 = await engine.check(req({
  ...
  const d2 = await engine.check(req({
    ruleKey: "git status $(rm -rf /tmp/x)",
...
test("权限: 复杂 shell 在 bypass 下也压不过已配置 deny 规则", async () => {
  ...
  const decision = await engine.check(req({
