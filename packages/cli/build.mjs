...
//  2. externalize：凡是「裸依赖且不属于 @agentx/*」的都留作外部依赖（ink/react/sdk/openai
//     由发布包的 dependencies 在安装时提供）；@agentx/* 与相对路径全部内联，
...
const externalizePlugin = {
  ...
  setup(b) {
    b.onResolve({ filter: /.*/ }, (args) => {
      ...
      if (p.startsWith("@agentx/")) return null; // 内联工作区包（经 exports 解析到 src）
